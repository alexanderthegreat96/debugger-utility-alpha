...
class DebuggerUtility
{
    ...
    protected static function renderDump($value, $level, $plainText, $ansiColors)
    {
        ...
        if (is_string($value)) {
            ...
            $dump = sprintf(
                '%s (%s)',
                OutputUtility::ansiEscapeWrap($value, '35', $ansiColors),
                gettype($value)
            );
        } elseif (is_bool($value)) {
            $dump = $value ? OutputUtility::ansiEscapeWrap('TRUE', '32', $ansiColors) :
               OutputUtility::ansiEscapeWrap('FALSE', '32', $ansiColors);
